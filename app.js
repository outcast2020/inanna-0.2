...
(function () {
  ...
  function maxX() {
    return Math.max(minX(), window.innerWidth - catWidth() - 12);
  }
  ...
  function setFacing(nextDirection) {
    const facing = nextDirection >= 0 ? -1 : 1;
    shell.style.setProperty("--cat-facing", String(facing));
  }

  function setMode(mode) {
    cat.classList.toggle("is-idle", mode === "idle");
    cat.classList.toggle("is-walking", mode === "walking");
  ...
  function flashCounter() {
    if (!counter || !petSpan) return;
    petSpan.textContent = String(petCount);
  ...
  function setWalking(nextWalking) {
    if (isWalking === nextWalking) return;
